  ...
  return (
    <main className="mx-auto max-w-3xl p-6">
      ...
      <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <h1 className="mb-4 text-2xl font-semibold leading-tight text-slate-900">
          {ticket.title}
        </h1>

        <div className="mb-6 flex flex-wrap gap-x-6 gap-y-1 text-sm text-slate-600">
          <div>
            <span className="text-slate-400">Auteur</span>{" "}
            <span className="font-medium text-slate-700">{ticket.author_name ?? "—"}</span>
          </div>
          <div>
            <span className="text-slate-400">Créé le</span>{" "}
            <span className="font-medium text-slate-700">{formatDateTime(ticket.created_at)}</span>
          </div>
        </div>
        ...
        <div className="mb-4 flex flex-col gap-3">
          {appComments.length === 0 ? (
            ...
            appComments.map((c) => (
              ...
              >
                <div className="mb-1 flex items-center justify-between text-xs text-slate-500">
                  <span className="font-medium capitalize text-slate-700">{c.author_name ?? "—"}</span>
